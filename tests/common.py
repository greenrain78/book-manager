...
from src.repository.manager import UsersRepository, BooksRepository, BorrowRepository, BorrowHistoryRepository, \
...
class SystemTestBase(unittest.TestCase):
    ...
    # borrow_prompt 실행 전 AppContext + Service 설정
    def prepare_test_context(self, file_data):
        # 테스트용 파일 작성
        self.write_files(file_data)
        # patch context 생성을 테스트 객체에 저장
        self.patches = [
        ...
        # 강제 주입
        app.users_repo = UsersRepository(path=self.paths["users"])
        app.books_repo = BooksRepository(path=self.paths["books"])
        app.borrow_repo = BorrowRepository(path=self.paths["borrow"])
        app.borrow_history_repo = BorrowHistoryRepository(path=self.paths["borrow_hist"])
        app.isbn_repo = ISBNRepository(path=self.paths["isbn"])
        app.cat_repo = CategoryRepository(path=self.paths["cats"])
    ...
    # borrow_prompt 실행 + 출력 캡처
    @staticmethod
    def execute_app(input_values, app: AppContext):
        try:
            with patch("builtins.input", side_effect=input_values):
                buffer = io.StringIO()
                with patch("sys.stdout", buffer):
            ...
            print(f"\n[Error] 입력값이 부족하여 프롬프트 실행이 중단되었습니다.\n{output}")
            return output
