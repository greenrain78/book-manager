...
class BaseRepository:
    ...
    def _validate_line_common(self, raw: str) -> None:
        ...
        # 필드 구분자(|) 개수 검사 - 파이프(|) 개수가 기대한 필드 수와 다르면 에러 발생
        if raw.count("|") != self.expected_fields:
    ...
    def load_all(self) -> List[Any]:
        ...
        with open(self.path, "r", encoding="utf-8", newline="") as fp:
            for line in fp:
                ...
                fields = raw.split("|")
                items.append(self.factory_from_fields(fields))
        return items

    def save_all(self, items: Iterable[Any]) -> None:
    ...
    def to_list_of_dicts(self) -> List[Dict[str, Any]]:
        return [getattr(it, "to_dict")() for it in self.load_all()]
